...
class EditorSettingsWidget(AbstractSettingsWidget):
    def __init__(self) -> None:
        ...
        self.font_family_changer = AbstractSettingFrame(
        ...
        self.font_family_combo.currentTextChanged.connect(
            lambda fam: EditorFontManager.set_current_font(family=fam)
        )

        self.font_size_changer = AbstractSettingFrame(
        ...
        self.font_size_spin.valueChanged.connect(
            lambda value: EditorFontManager.set_current_font(size=value)
        )

        self.cursor_style_changer = AbstractSettingFrame(
            "Cursor Style", "Defines the cursor style"
        )
        self.cursor_style_combo = self.cursor_style_changer.add_widget(
            DropDownMenu("line", "block")
        )
        self.cursor_style_combo.currentTextChanged.connect(
            lambda style: EditorSettingsUpdater.set_cursor_style(style)
        )

        self.tab_width_changer = AbstractSettingFrame(
            "Tab Width", "Defines the tab width"
        )
        self.tab_width_combo = self.tab_width_changer.add_widget(
            DropDownMenu(*[str(i) for i in range(2, 9)], width=50)
        )
        self.tab_width_combo.currentTextChanged.connect(
            lambda wid: EditorSettingsUpdater.set_tab_width(int(wid))
        )
